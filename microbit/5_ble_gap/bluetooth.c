...
uint32_t bluetooth_gap_advertise_start(){
	...
	static uint8_t adv_data[] = {
		// Add some stuff
		 16,BLE_GAP_AD_TYPE_COMPLETE_LOCAL_NAME,
		'H','O','R','E','R',' ','E','R',' ','G','R','E','I','T','!'

	};
	uint8_t adv_data_length = 17;

	err_code = sd_ble_gap_adv_data_set(adv_data, adv_data_length, NULL, 0);

	// Add more stuff
	ble_gap_adv_params_t adv_params;	//Lager argument struct
	memset(&adv_params, 0, sizeof(adv_params)); //Nullstiller alt i structen
	//Setter Type(0) og Interval
	adv_params.type = BLE_GAP_ADV_TYPE_ADV_IND;
	adv_params.interval = 0x0040;  		//Setter intervall
